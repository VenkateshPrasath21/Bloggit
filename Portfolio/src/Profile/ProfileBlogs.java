 ...
 */
@WebServlet("/ProfileBlogs")
public class ProfileBlogs extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getSession().getAttribute("userid")==null){
			...
			try {
				statement = GetConnection.getInstance().prepareStatement("select blogid,profile.imageurl,blogs.imageurl,profile.userid,dateadded,title,username,views,blogs.topics,body from blogs INNER JOIN profile ON blogs.userid = profile.userid where blogs.userid = ? ORDER BY views DESC");
				...
				ResultSet rs = statement.executeQuery();
		         ...
		         while(true){
		        	if(rs.next()) {
		        		...
		        		titleObj.put("link","/Portfolio/Blogs/"+ rs.getString("blogid"));
		        		titleObj.put("AUTHOR_NAME", rs.getString("username"));
